...
function Testimonial({ photo, quote, name, title }: { photo: string; quote: string; name: string; title: string }) {
  return (
    <div className="bg-[#f7f5f1] rounded-2xl p-6 border border-[rgba(43,51,53,0.08)] flex flex-col gap-4">
      <p className="text-[15px] text-[#43382f] leading-relaxed italic">&ldquo;{quote}&rdquo;</p>
      <div className="flex items-center gap-3">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={photo} className="w-10 h-10 rounded-full object-cover flex-shrink-0" alt={name} />
        <div>
          <p className="text-[13px] font-semibold text-[#2b3335] leading-snug">{name}</p>
          <p className="text-[12px] text-[#677283] leading-snug">{title}</p>
        </div>
      </div>
    </div>
  );
    ...
    <section style={{ background: '#ffffff' }} className="w-full">
      ...
      >
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
          ...
          <div className="hidden lg:flex lg:w-[380px] lg:flex-shrink-0 items-start">
            <Testimonial
...
This must render correctly at all breakpoints. On mobile and tablet, the testimonial right columns are hidden. Do not change any other file.